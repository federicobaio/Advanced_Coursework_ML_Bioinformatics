...
For comments and questions, contact me: giovanni.donghi@usi.ch
...
You have to implement the following modules:
- The ReLU activation function
- A learnable linear layer
- The Softmax+Cross-Entropy loss

The linear layer is also called fully connected layer. The bias term is not included in the
...
All the functions you have to implement have their signature pre-specified, with annotated types. You should _not_ change this, because it will be tested by an automated system which depends on the current interface.
...
All the modules have a `forward()` and a `backward()` method. `forward()` receives one argument (except for the loss which takes two) and
...
class ReLU(Layer):
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        result = np.maximum(0,x) # The ReLU is max(0,x)
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        d_x = grad_in*(relu_x>0) # Where (relu_x>0)is basically the indicator function, i.e. d ReLU(x)/dx
...
class Linear(Layer):
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        y = x@W + b # y have dimension (batch_size,output_size)
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        dW = x.T@grad_in # We must write in this order since x have dimension (batch_size,input_size) and grad_in is
                         # (batch_size, output_size), so that we have same dimensionality of W,
                         # which is (input_size,output_size)
        db = np.ones(grad_in.shape[0])@grad_in # create a ones vector of dimension=batch_size

        d_inputs = grad_in@self.var['W'].T # here we have the same dimension of x since we have dx that is (batch_size,input)
...
class SoftmaxCrossEntropy(Loss):
    def forward(self, inputs: np.ndarray, targets: np.ndarray) -> float:
        ...
        ## Implement
        ## Remember that here targets are not one-hot vectors, but integers specifying the class.
        ...
        one_hot_t = np.zeros((n,num_classes)) # we create the empty matrix of 0
        one_hot_t[np.arange(n),t] = 1 # we fill with one using the class specified by t
        softmax = np.exp(y)/np.sum(np.exp(y),axis=1,keepdims=True)

        mean_crossentropy = -np.sum(one_hot_t * np.log(softmax))/n
        # since has to return a single scalar we divide by n, the batch_size
    ...
    def backward(self) -> np.ndarray:
        ...
        d_inputs = (softmax - one_hot_t)/softmax.shape[0] # the formula of the derivative of
                                                          # the cross entropy and softwax is just this one
                                                          # note that since we evaluated above the mean_crossentropy
                                                          # we have to divide by n again, but we do not have the
                                                          # variable n (batch_size) since is been defined in the function
                                                          # above, and we cannot modify anything outside the Implement-End
                                                          # area, so we use the soft_max.shape[0] that is exactly (by definition
                                                          # of softmax (batch_size,num_classes)) the number of batch
...
def train_one_step(model: Layer, loss: Loss, learning_rate: float, input: np.ndarray, target: np.ndarray, weight_decay=0.0005) -> float:
    ...
    loss_value = loss.forward(model.forward(input), target) # we first with model.forward obtain the y that we use
                                                            # to obtain the actual value of the loss

    variable_gradients = model.backward(loss.backward()).variable_grads # first we evaluate our "grad_in" of the last layer:
                                                                        # loss.backward() and then backpropagate for other layers
                                                                        # we extract only the variable_grads since we want to update W and b
                                                                        # using all the gradients (the input_grads is used as input to
                                                                        # continue the backpropagation)

    for key, grad in variable_gradients.items():
      penalization = weight_decay * model.var[key] # the gradiente of the additional loss term w.r.t the variables
      model.var[key] = model.var[key] - learning_rate * (grad + penalization)
...
In this part you will use your ML framework to train a network that can classify handwritten digits. The network is a very simple 2 layer network, with layer output sizes of 20 and 10 respectively. Again, the skeleton is given, and you have to fill in the missing parts. After this part, you will have a rough idea how to build a proper training pipeline. Pay attention to the description in the skeleton file: it clarifies your task further, and also gives some useful hints. Reuse as much as you can from the framework you defined in the first part of the assignment, by using the corresponding functions and classes (e.g the layers, the `train_one_step()` function, etc).
...
train_indices = indices[:n_train] # i.e we take the first 70% of the indices as training
...
def verify(images: np.ndarray, targets: np.ndarray) -> Tuple[int, int]:
    ...
    predictions = np.argmax(model.forward(images), axis=1) # the max value at the end of the forward along
                                                           # probability of each class
    num_ok = np.sum(predictions==targets)
...
def test() -> float:
    ...
    for i in range(0, len(test_set), batch_size):
        ...
        num_ok = verify(images,labels)[0] # the first item is the num_ok
        total_num = verify(images,labels)[1] # the second item is the total_num
        accumulated_ok = accumulated_ok + num_ok
...
def validate() -> float:
    ...
    for i in range(0, len(validation_indices), batch_size): # we start by n_train since the first 70% are training
        ...
        num_ok = verify(images,labels)[0]
        total_num = verify(images,labels)[1]
        accumulated_ok = accumulated_ok + num_ok
        count = count + total_num
...
for epoch in range(200):
    ...
    for i in range(0, len(train_indices), batch_size):
        ...
        current_loss_value = current_loss_value + train_one_step(model, loss, learning_rate, images, labels)

    current_loss_value = current_loss_value / (n_train / batch_size)  # media loss per batch
